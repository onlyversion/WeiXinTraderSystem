...
namespace BlackAmberSystem
{
    ...
    public partial class AboutBox : Window {
        ...
        public string Copyright {
            get
            {
                string copyright = CalculatePropertyValue<AssemblyCopyrightAttribute>(propertyNameCopyright, xPathCopyright); 
                return copyright;
            }
        }
        ...
        public string Company {
            get
            {
                string company= CalculatePropertyValue<AssemblyCompanyAttribute>( propertyNameCompany, xPathCompany );
                return company;
            }
        }
        ...
        /// 从一个特定的属性或资源字典获取指定的属性值。
        /// </summary>
        /// <typeparam name="T">Attribute type that we're trying to retrieve.我们要检索的属性类型</typeparam>
        /// <param name="propertyName">Property name to use on the attribute.使用的属性名称</param>
        /// <param name="xpathQuery">XPath to the element in the XML data resource. XML数据源的路径</param>
        /// <returns>The resulting string to use for a property.结果字符串是使用的一个属性
        /// Returns null if no data could be retrieved.没有值则返回Null</returns>
        private string CalculatePropertyValue<T>( string propertyName, string xpathQuery ) {
            string result = string.Empty;
            // first, try to get the property value from an attribute.首先，尝试从一个属性中得到属性值
